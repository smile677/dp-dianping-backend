    ...
    private class VoucherOrderHandler implements Runnable {
        ...
        public void run() {
            while (true) {
                try {
                    // 1. 获取消息队列中的订单信息
                    // XREADGROUP GROUP g1 c1 COUNT 1 BLOCK 2000 STREAMS seckill:order >
        ...
        private void handlePandingList() {
            while (true) {
                // 直到pending-list中的所有异常处理完成为止
                try {
                    // 1. 获取pending-list中的订单信息
                    // XREADGROUP GROUP g1 c1 COUNT 1  STREAMS seckill:order 0
                    List<MapRecord<String, Object, Object>> list = stringRedisTemplate.opsForStream().read(
                            Consumer.from("g1", "c1"),
                    ...
                    // 2. 判断消息获取是否成功
                    if (list == null || list.isEmpty()) {
                        // 2.1 如果失败说明没有pending-list消息，继续循环
                        break;
                    }
                    // 3. 解析消息中的订单
                    MapRecord<String, Object, Object> record = list.get(0);
